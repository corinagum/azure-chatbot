...
namespace BugDialogBot
{
    ...
    class BugReportDialog : IDialog<object>
    {
        ...
        Node test = new Node() { question = "Welcome to Azure Support bot! Which deployment model did you use?", answers = new Dictionary<int,string> { { 1,"Resource" }, { 2,"Classic" } } };
        ...
        public async Task ResumeAndPromptPlatformAsync(IDialogContext context, IAwaitable<string> argument)
        {
            ...
            Node test1 = new Node() { question = @"'You can reset credentials or SSHD using either Azure CLI commands directly or using the Azure VM Access for Linux extension. 
                How would you like to resolve the issue?'", answers = new Dictionary<int, string> { { 1, "First answer" }, { 2, "Second answer" }, { 3, "Third answer" }, { 4, "fourth answer" }, { 5, "fifth answer" } } };
